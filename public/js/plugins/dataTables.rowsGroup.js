...
RowsGroup.prototype = {
	...
	_mergeColumn: function(iStartRow, iFinishRow, columnsIndexes)
	{
		...
		var getRowSpan = parseInt($(columnNodes[newSequenceRow]).attr('rowspan'));

		if( getRowSpan > 1 ) {
			/*var getIndex = $('.publicDataTable tbody tr').index($(columnNodes[newSequenceRow]).closest('tr'));

			var addIndex = getIndex+getRowSpan;
			var addHighlight = '<tr>';
				addHighlight += '	<td colspan="5">합계</td>';
				addHighlight += '	<td></td>';
				addHighlight += '	<td></td>';
				addHighlight += '	<td></td>';
				addHighlight += '	<td></td>';
				addHighlight += '	<td>ㅇㅇㅇㅇㅇㅇ</td>';
				addHighlight += '</tr>';

			$('.publicDataTable tbody tr')
				.eq(addIndex)
				.after(addHighlight);*/
		}
